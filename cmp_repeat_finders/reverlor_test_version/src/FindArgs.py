...
FINDER_CHOICES = ('minimap2', 'repeat-scout', 'phraider', 'total-repeats', 'repeat-modeler', 'grf', 'reprise')
...
def _add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Path to REPrise executable'
    )
    parser.add_argument(
        '--java',
        type=str,
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if args.finder == 'repeat-scout':
        ...
        _validate_reprise(args.reprise)
    else:
...
def _validate_reprise(reprise_fpath: Optional[str]) -> None:
    if reprise_fpath is None:
        sys.stderr.write('Error: REPrise executable is not provided\n')
        sys.stderr.write('Please provide it with the --reprise option\n')
        sys.exit(1)
    # end if
    if not os.path.isfile(reprise_fpath):
        sys.stderr.write(f'Error: REPrise executable `{reprise_fpath}` does not exist\n')
        sys.exit(1)
    # end if
    if not os.access(reprise_fpath, os.X_OK):
        sys.stderr.write(f'Error: REPrise executable `{reprise_fpath}` is not executable\n')
        sys.exit(1)
    # end if
# end def
...
class FindArgs:
    def __init__(self,
                 ...
                 reprise_fpath: Optional[str] = None,
                 java_fpath: str = 'java',
                 keep_tmp: bool = False,
        ...
        self.reprise_fpath: str = reprise_fpath
